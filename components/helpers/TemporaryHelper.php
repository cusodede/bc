...
class TemporaryHelper {
	...
	/**
	 * Использовать осторожно, при выводе полного и глубокого трейса будет жрать память практически безгранично.
	 * @param int $options
	 * @param int $limit
	 * @return string
	 */
	public static function DebugTrace(int $options = 0, int $limit = 10):string {
		$trace = debug_backtrace($options, $limit);
		$trace = array_reverse($trace);
		array_pop($trace); //Убрать текущий вызов
		$result = [];

		foreach ($trace as $i => $value) {
			$args = [] === $value['args']?'':print_r($value['args'], true);
			$class = $value['class']??'';
			$file = $value['file']??'?';
			$line = $value['line']??'?';
			$type = $value['type']??'';
			$traceData = "($i) {$file}:{$line} => {$class}{$type}{$value['function']}($args)";
			if (isset($value['object'])) $traceData .= print_r($value['object'], true);
			$result[] = $traceData;
	...
	/**
	 * Преобразует строку в camelСase
	 * @param string $string
